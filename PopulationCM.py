...
class MainWindow(QMainWindow): #Main window
    ...
    def show_p_value_popup(self):
        self.msg = QMessageBox()
        self.msg.setIcon(QMessageBox.Information)
        self.msg.setWindowTitle('PopulationCM. P-Value')
        self.msg.setWindowIcon(QIcon('Icon.ico'))
        self.msg.setText("P-Value is equal to " + str_p_value)
...
def l(arr1, arr2):  # Линейная регрессия
    global str_p_value
    # arr1 - x; arr2 - y
    df = pd.DataFrame(list(zip(arr1, arr2)), columns=['x', 'y'])
    model = smf.ols('y ~ x', data=df).fit()
    str_p_value = str(round(model.pvalues[1],4))
